...
export default class ActDia {
  ...
  updateItemShapeAndListeners(shape, svgShape, item) {
    if (shape.id !== svgShape?.id) {
      //console.log(item.svgElement.innerHTML);
  ...
  getStyleSVGAttributes(style, options) {
    ...
    if (style.rotate) {
      ...
      if (Array.isArray(rotate))
        ...
        rotate = `${rotate} ${style.rotateCenterX || 0} ${style.rotateCenterY || 0}`;
  ...
  getShapeSVGData(shape, item, options = {}) {
    ...
    switch (shape.shape) {
      ...
      default:
        ...
        if (rotate) {
          let rotateParam = rotate;
          if (Array.isArray(rotateParam))
            rotateParam = rotateParam.join(' ');
          else
            rotateParam = `${rotateParam} ${rotateCenterX || 0} ${rotateCenterY || 0}`;

          attributes.transform += ` rotate(${rotateParam})`;
        }
